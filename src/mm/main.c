...
for (i = 0; i < NR_PROCS; ++i){
    /* iterujemy od 0 do wielkosci tabeli procesow mm_in.m1_i1 */
    /* na zajeciach bylo zeby sprawdzac IN_USE */

    if((mproc[i].mp_pid == pid) && ((mproc[i].mp_flags & IN_USE) != 0)){
        return i;
        }
    }
    return ENOENT;
...
PUBLIC int do_setprocrate(){
  message m = mm_in;
  return _taskcall(SYSTASK, SYS_SETPROCRATE, &m);
}
