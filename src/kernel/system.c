...
FORWARD _PROTOTYPE( int do_setprocrate, (message *m_ptr) );
...
/* Main entry point of sys_task.  Get the message and dispatch on type. */
  ...
  while (TRUE) {
	...
	switch (m.m_type) {	/* which system call */
      ...
      case SYS_SETPROCRATE: r = do_setprocrate(&m); break;
	    default:		r = E_BAD_FCN;
	}
...
register message *m_ptr;	
{
  ...
  new_proc_group = m_ptr->m1_i2;
  if (new_proc_group != 1 && new_proc_group !=0){
    return EINVAL;
  }
...
PRIVATE int do_setprocrate(m_ptr)
register message *m_ptr;
{
  if (m_ptr->m1_i2 < 0 || m_ptr->m1_i2 > 100){
    return EINVAL;
  }
  if (m_ptr->m1_i1 != 1 && m_ptr->m1_i1 != 0)
  {
    return EINVAL;
  }
  if (m_ptr->m1_i1 == 0)
  {
    MAX_PROC_A = m_ptr->m1_i2;
  }
  if (m_ptr->m1_i1 == 1)
  {
    MAX_PROC_B = m_ptr->m1_i2;
  }
  return OK;
}
