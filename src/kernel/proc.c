...
int GROUP_A_COUNTER = 0;
int GROUP_B_COUNTER = 0;
MAX_PROC_A = 1;
MAX_PROC_B = 3;
 ...
 */
  ...
  if ( (rp = rdy_head[USER_Q]) != NIL_PROC) {
    if (GROUP_A_COUNTER == 0 && GROUP_B_COUNTER == 0){
      proc_ptr = rp;
      bill_ptr = rp;
      return;
    } else {
      ...
      rdy_head[USER_Q]->p_nextready = rp;
      rp->p_nextready = temp;
 ...
 */
  ...
  rdy_tail[USER_Q]->p_nextready = NIL_PROC;
  if (rdy_head[USER_Q]->group == 0){
      GROUP_A_COUNTER = MAX_PROC_A;
    } else {
      GROUP_B_COUNTER = MAX_PROC_B;
    }
  }
  if (rdy_head[USER_Q]->group == 0)
    {
      GROUP_A_COUNTER--;
      if (GROUP_A_COUNTER == 0)
      {
      pick_proc();
      return;
      }
    } else {    /* assuming all user procs are in one of groups*/
        GROUP_B_COUNTER--;
      if (GROUP_B_COUNTER == 0)
        {
        pick_proc();
        return;
        }
